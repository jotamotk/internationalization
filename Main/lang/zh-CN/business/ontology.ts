export default {
  ...
  copyClass: {
    ...
    copyMany: '批量复制',
    tip: '复制的类的属性和约束将应用于其他类。',
    uploadByExcel: '通过Excel上传',
    template: '模板',
    addSuccess: '成功添加 ({number}) 个类',
    addSuccessSome:
    '成功添加 ({successNum}) 个类，但在添加 ({failNum}) 个类时遇到了问题。',
    addFail: '无法从您的Excel中获取信息，请检查您的文件',
    overExcelToast: '一次最多只能添加 999 行',
    emptyToolType: 'ToolType不能为空',
    duplicatedName: '类名不能重复',
    successCopied: '成功复制 ({number}) 个类',
    successCopiedPart:
    '成功复制 ({successNum}) 个类，但在复制 ({failNum}) 个类时遇到了问题。',
    failCopied: '无法复制类',
    },
};
